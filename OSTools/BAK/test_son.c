...
/*	VIRTUAL_FILE *f;
	f = VirtualFileOpen((void*)"hehe.txt", 0, VF_AUTO, VF_O_WRITE);
	VirtualFilePuts("It's me, ", f);
	VirtualFilePuts("mario!\nTest2", f);
	VirtualFileClose(f);*/

/*	void MODPlay			(const void *modFile);
	void oslInitAudioMOD();

	oslInitAudioMOD();
	mod = oslLoadSoundFileMOD("test.mod", 0);
	oslPlaySound(mod, 0);*/

/*
	snd = oslLoadSoundFileBGM("music.bgm", OSL_FMT_STREAM);
//	snd = oslLoadSoundFileWAV("key.wav", OSL_FMT_STREAM);
	oslPlaySound(snd, 1);
	oslSetSoundLoop(snd, 1);*/
//	oslDebug("%i", theVariable);
